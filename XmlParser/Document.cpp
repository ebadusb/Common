...
*
*  Implementation of the Document XML class
*/
...
Document::Document(const std::string& sFile) :
   _fileName(sFile),
   _isStandAlone(false),
   _fileMissing(false),
...
bool Document::read()
{
   ...
   if (!err && _pRootElement != NULL) bReadSuccess = true;
   else bReadSuccess = false;
   return bReadSuccess;
}
...
void Document::write()
{
   if (_pRootElement->isDirty()) write(_fileName);
}
...
void Document::toStream(std::ostream& output) const
{
   if (_hasDocDecl)
   {
      if (_isStandAlone) output << "<?xml version=\"" << _version << "\" encoding=\"" << _encoding << "\" ?>";
      else output << "<?xml version=\"" << _version << "\" encoding=\"" << _encoding << "\" standalone=\"no\" ?>";
   }
   if (!(_pRootElement == NULL)) getRootElement()->toStream(output);

# $Header: K:/BCT_Development/vxWorks/Common/rcs/makefile.frm 1.1 2007/03/13 18:05:31Z jmedusb Exp jl11312 $
# $Log: makefile.frm $
# Revision 1.1  2007/02/07 17:29:51Z  jmedusb
# Initial revision

PRODUCTION_RMSG_DIR := \\bctquad3\HOME\BCT_Development\Taos\reserved_messages_builds
PRODUCTION_RMSG_DIR_ECHO := $(subst \,/, $(PRODUCTION_RMSG_DIR))
LRMBD := latest_rmsg_build_dir.mk

.PHONY: find_rmsg
find_rmsg:
	@echo Finding the current reserved messages directory
	@rm -f -r .rmsg
	@mkdir .rmsg
	@if exist $(LRMBD) (echo Default Reserved Message directory overridden\n)
	@if exist $(LRMBD) (echo $(subst .,$(shell cmd /c cd), $(shell cmd /c if exist $(LRMBD) cat $(LRMBD))) > .rmsg\$(LRMBD))
...
.PHONY: verify_rmsg
verify_rmsg:
	@echo Verifying the current reserved messages directory
	@rm -f .rmsg/status

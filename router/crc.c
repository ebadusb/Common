 ...
 *
 * $Header: I:/trima_project/source/router/rcs/crc.c 1.2 1996/07/24 19:49:20 SS03309 Exp $
 * $Log: crc.c $
...
{
   SHORT_MSG_HDR* hdr = (SHORT_MSG_HDR*) blk;
   unsigned long crc = INITIAL_CRC;
   unsigned short blk_len = BLOCK_LEN;
   unsigned char* blk_adr = (unsigned char*) blk;

   while( blk_len--)                      // compute on first part
   {
      crc = crctable[ (crc ^ *blk_adr++) & 0xFFL] ^ (crc >> 8);
   }
   blk_adr++;                             // skip CRC
...
// compute on rest of message

   blk_len = hdr->length - (unsigned short) 8;
   while( blk_len--)
   {
      crc = crctable[ (crc ^ *blk_adr++) & 0xFFL] ^ (crc >> 8);
   }

   crc = crc ^ INITIAL_CRC;               // invert CRC
   return ( crc == hdr->CRC);             // return match
}
...
void updateFocusMsgCRC( void* blk)
{
   ...
   while( blk_len--)                // compute first part
   {
      crc = crctable[ (crc ^ *blk_adr++) & 0xFFL] ^ (crc >> 8);
   }
   blk_adr++;                       // skip CRC
   ...
   blk_len = hdr->length - 8;       // compute rest
   while( blk_len--)
   {
      crc = crctable[ (crc ^ *blk_adr++) & 0xFFL] ^ (crc >> 8);
   }

   hdr->CRC = crc ^ INITIAL_CRC;    // invert CRC
}

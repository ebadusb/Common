...
// range checks
   ...
   if (mhdr == NULL || mhdr->msgID == 0)
   {
      // Message has never been registered, so skip deregistering.
      // This case is generated by the CStdMsg class, since
      // these messages aren't registered until they are actually
      // sent, but the destructor chain ends up calling
      // registerMessage anyway.
      return;
   }

...
int test1_main(void)
{
   ...
   // Verify that the test1_a.dat file doesn't contain the override-user-config tag. It should be ingnored
   // because it's a read-only file.
   // Verify that the test1_m.dat contains the override tag for the section1 param1 entry
   // Verify that the test1_m.dat file can be read and that the section1 param1 value is as expected.
   if ( checkUserConfigOverride(Test1::Test1_C_Access.defaultFileName(), "Section2", "Param2")
        || checkUserConfigOverride(Test1::Test1_A_Access.fileName(), "Section2", "Param7")
        || !checkUserConfigOverride(Test1::Test1_M_Access.defaultFileName(), "Section1", "Param1")
        || ConfigFile::ReadDefaultOK != Test1::Test1_M_Access.readFile(&log_level_config_data_info, &log_level_config_data_error)
        || Test1::Test1_M().Section1.Param1 != 0.4)
   {
      fprintf(stderr, "test failed\n");
      return -1;
   }

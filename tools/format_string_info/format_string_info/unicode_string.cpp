...
UnicodeString::UnicodeString(const StringChar * ucsStr)
{
	...
	if( ucsStr )
	{
		allocate( (unsigned int)ucslen( ucsStr ) );
...
int UnicodeString::insert(const StringChar * ins, int index)
{
	if( ins )
	{
		unsigned int nsize = (unsigned int)ucslen( ins );
...
int UnicodeString::insert(const UnicodeString& ucsStr, int index)
{
	unsigned int nsize = ucsStr._length;
...
int UnicodeString::find(const StringChar a, int index) const
{
	...
	if( _data )
	{
		if( index >= 0 && index < (int)_length )
		{
			for( int i=index; i<(int)_length; i++ )
			{
				if( a == _data[i] )
				{
					retVal = i;
...
int UnicodeString::reverseFind(const StringChar a, int index) const
{
	...
	if( _data )
	{
		...
		if( nIndex >= 0 && nIndex < (int)_length )
		{
...
int UnicodeString::reverseFind(const StringChar *ucsStr, int index) const
{
	...
	if( _data && ucsStr )
	{
		int len = (int)_length;
		int nIndex = index;
		int findStrLen = (int)ucslen( ucsStr );
...
int UnicodeString::removeString(const StringChar *ucsStr, int index)
{
	...
	if( retVal >= 0 )
	{
		unsigned int len = (unsigned int)ucslen( ucsStr );
...
std::string UnicodeString::getUTF8(void) const
{
	std::string retVal;
...
int UnicodeString::replace( StringChar chOld, StringChar chNew )
{
	...
	if( _data && _length > 0 )
	{
		...
		_length = (unsigned int)ucslen( _data );
	}

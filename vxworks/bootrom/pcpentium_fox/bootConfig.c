...
12v,03jun03,wap  Add call to ifIndexLibInit() (which is now required)
...
#define TIMEOUT		3	/* number of seconds before auto-boot */
...
LOCAL char  autoboot (int timeout,int kbdFound);
LOCAL void  printBootLogo (void);
LOCAL void  bootHelp (void);
LOCAL STATUS   bootLoad (char *bootString, FUNCPTR *pEntry, BOOL kbdPresent, BOOL altBoot);
...
LOCAL STATUS   getArg ();
#ifdef OPTIA_BOOTROM
LOCAL int bootPauseButtonPressed();
LOCAL int bootStopButtonPressed();
#endif /* OPTIA_BOOTROM */
...

{
    ...
    * I/O port 0x64 as a status port.  Port 0x64 has the following bit
    * assignments.
    *
   ...
   */
   enum { DataReg = 0x60 };
   enum { StatusReg = 0x64 };
   enum { RcvDataAvail = 0x02 };

   /*
    * We check for the keyboard by sending a diagnostic echo command.
    * If a keyboard is present, it will send the same code back
    * as a response.
   */
	...
	*sysExcMsg = EOS;		  /* indicate exception message is old */
	...
	for ( retry=0 ; retry<10 && !kbdFound ; retry++ )
	{
		...
		{
			if ( sysInByte((int)DataReg) == EchoCommand )
			{
				kbdFound = 1;
			}
		}
        ...
        !(sysFlags & SYSFLG_NO_AUTOBOOT) )
   {
      ...
      if ( !kbdFound )
        key = autoboot ( 2, kbdFound );  /* doesn't return if successful */
      else 
		key = autoboot ( timeout, kbdFound );	/* doesn't return if successful */
	}
...
(%d bytes). Not saved to NVRAM\n", (int) BOOT_LINE_SIZE);
			...
			case '$':		/* load and go with internal params */
				...
				if ( bootLoad (pLine, &entry, kbdFound, 0) == OK )
				{
					go (entry);
				}
				else
...
LOCAL char autoboot 
		...
		int kbdFound	/* flag to show whether the keyboard was found */
		)
{
   ULONG   autoBootTime;
   int     timeLeft;
   ...
   BOOL    altboot = 0;
	...
	if ( timeout > 0 )
	{
		if ( kbdFound ) printf ("Press <ESC> key for alternate boot ...\n");
		...
		if ( kbdFound ) printf ("%2d\r", timeLeft);
		...
		while ( (tickGet () < autoBootTime) && (bytesRead == 0) )
		{
			if ( kbdFound )
			{
				...
				if ( bytesRead > 0 )
				{
					...
					if ( keypressedno == 1 && key == 0x1b /* ESC key */ )
					{
					...
					if ( ( keypressedno == 1 && key == 'd' ) ||
						 ( keypressedno == 2 && key == 'b' ) )
						bytesRead = 0;
					else if ( keypressedno == 3 && key == 'g' )
						return(' ');
					else
						break;
				}
		...
		)
{
   ...
   (void) sysNvRamSet (BOOT_LINE_ADRS,
	...
	if ( strncmp (params.bootDev, "ata", 3) == 0 )
	{
		...
		/* delay a couple of seconds to allow the disk to startup correctly */
		ULONG   autoBootTime;
		autoBootTime = tickGet () + sysClkRateGet () * 2;
		while (tickGet () < autoBootTime );
...
#ifdef OPTIA_BOOTROM
	if ( (fd = open ("/ata10/boot.plt", O_RDONLY, 0)) != ERROR )
	{
		if ( !bootStopButtonPressed() || !bootPauseButtonPressed() )
		{
			strcat(fileName,".plt");
		}
		/* else -> load the normal optia kernel */
	}
#endif /* OPTIA_BOOTROM */
